...
export class CopytradeArbBot {
    ...
    private validateConfig(): void {
        ...
        if (this.cfg.maxSumAvg >= 1.0) {
            warnings.push(`⚠️ MAX_SUM_AVG (${this.cfg.maxSumAvg}) is >= 1.0, which means no profitable trades will be allowed!`);
        }

        if (this.cfg.maxSumAvg < 0.9) {
            warnings.push(`⚠️ MAX_SUM_AVG (${this.cfg.maxSumAvg}) is very low, may skip many trades`);
        }

        if (this.cfg.sharesPerSide < 1) {
            warnings.push(`⚠️ SHARES_PER_ORDER (${this.cfg.sharesPerSide}) is less than 1`);
        }

        if (this.cfg.minBalanceUsdc < 1) {
            warnings.push(`⚠️ MIN_BALANCE_USDC (${this.cfg.minBalanceUsdc}) is less than $1, may cause issues`);
        }

        if (this.cfg.maxDrawdownPercent > 0 && this.cfg.maxDrawdownPercent > 100) {
            warnings.push(`⚠️ MAX_DRAWDOWN_PERCENT (${this.cfg.maxDrawdownPercent}) is > 100%`);
        }

        if (this.cfg.minPollMs >= this.cfg.maxPollMs) {
            warnings.push(`⚠️ MIN_POLL_INTERVAL_MS (${this.cfg.minPollMs}) >= MAX_POLL_INTERVAL_MS (${this.cfg.maxPollMs})`);
        }
    ...
    private async tickMarket(market: string): Promise<void> {
        ...
        // Timer starts when price enters range: (threshold - PRICE_BUFFER) <= price <= threshold
