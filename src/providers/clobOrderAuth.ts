...
/**
 * CLOB order signing mode + funder (Polymarket proxy) for @polymarket/clob-client.
 *
...
export async function resolveClobOrderAuth(wallet: Wallet): Promise<{
    ...
    let signatureType: SignatureType = SignatureType.POLY_GNOSIS_SAFE;
    ...
    if (signatureType === SignatureType.POLY_GNOSIS_SAFE && !config.funderWasExplicitInEnv) {
        console.log(
